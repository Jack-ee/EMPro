...
def _budget_start():
    """Arm the in-band deadline. Call once, at the start of a build."""
...
def run_build(dry_run=False, limit=0, cli_range=None, prune_voices=False):
    ...
    # sentence voice that is not in that list would be invisible to the app
    # even though its bytes are in the pack. The manifest therefore carries
    # the union, while `voices` stays the word-voice list the policy uses.
    ...
    gone     = {k: v for k, v in existing.items() if k[0] not in allset}
    kept     = {k: v for k, v in existing.items() if k[0] in allset}
    if gone:
        print("[prune] dropped %d clip(s) for entries removed from the list"
              % len(gone))

    # A clip whose voice is no longer used for its entry is dead weight: the
    # text-only rule above keeps it forever, so de-selected voices pile up
    # in the pack. Dropping them is opt-in (--prune-voices, or a
    # "# prune_voices: on" header) because it deletes audio that was paid
    # for, and re-selecting the voice means paying again.
    if prune_voices:
        stale = {k: v for k, v in kept.items()
                 if k[1] not in voices_for(k[0], voices, svoices)}
        if stale:
            mb = sum(len(v["audio"]) for v in stale.values()) / (1024 * 1024)
            for k in stale:
                del kept[k]
            print("[prune] dropped %d clip(s) in %d de-selected voice(s), "
                  "%.1f MB reclaimed"
                  % (len(stale), len(set(k[1] for k in stale)), mb))
    else:
        idle = set(k[1] for k in kept
                   if k[1] not in voices_for(k[0], voices, svoices))
        if idle:
            print("[prune] %d voice(s) no longer used for some entries are "
                  "still in the pack: %s. Re-run with --prune-voices to "
                  "reclaim that space."
                  % (len(idle), ", ".join(sorted(idle))))

    missing = collect_missing(build_entries, voices, kept, svoices)
    ...
    full_bytes, manifest = build_pack(all_clips, manifest_voices, new_gen, model)
    ...
    if delta_clips:
        delta_bytes, _ = build_pack(delta_clips, manifest_voices, new_gen, model)
    ...
    if aborted:
        ...
        #
        # The cause decides what the operator should do, so the two cases
        # get different advice. A spent time budget is a checkpoint and the
        # answer is simply "re-run"; anything else needs a look at the
        # account first, or the re-run stops at the same place.
        budget_stop = "time budget" in _abort_reason[0]
        if budget_stop:
            advice = (
                "  This is the planned checkpoint, not an error. Re-run the "
                "workflow to\n"
                "  continue: the next run downloads this pack and synthesises "
                "only what is\n"
                "  still missing, so nothing is paid for twice. Expect to "
                "re-run a few\n"
                "  times on a first full build.")
        else:
            advice = (
                ...
                "credit or raise\n"
                "  the limit, then re-run. The next run continues from the "
                "saved clip(s)\n"
                "  and only synthesises what is still missing.")
        raise SystemExit(
            "\n[INCOMPLETE] synthesis stopped early:\n"
            "  %s\n\n"
            "  The partial pack (generation %d, %d clip(s)) was still "
            "written\n"
            "  and will be published, so nothing already generated is "
            "lost.\n\n"
            "%s"
            % (_abort_reason[0], new_gen, manifest["clipCount"], advice))
...
def run_selftest():
    ...
    print("[selftest] OK - %d clips round-tripped, %d-byte pack, delta "
          "verified, voice policy + budget checked" % (len(fake), len(pack)))
