...
check('sw.js CACHE_NAME is emp-v115', sw.includes("const CACHE_NAME = 'emp-v115';"));
const vNew = (html.match(/\?v=115/g) || []).length;
const vOld = (html.match(/\?v=(9[89]|10[0-9]|11[0-4])"/g) || []).length;
check('index.html has 21 x ?v=115 and no stale versions',
      vNew === 21 && vOld === 0, vNew + ' new, ' + vOld + ' stale');
check('index.html loads reading-feeds.js',
      html.includes('<script src="reading-feeds.js?v=115"></script>'));
