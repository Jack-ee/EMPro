...
check('sw.js CACHE_NAME is emp-v122', sw.includes("const CACHE_NAME = 'emp-v122';"));
const vNew = (html.match(/\?v=122/g) || []).length;
const vOld = (html.match(/\?v=(9[89]|1[01][0-9])"/g) || []).length;
check('index.html has 21 x ?v=122 and no stale versions',
      vNew === 21 && vOld === 0, vNew + ' new, ' + vOld + ' stale');
check('index.html loads reading-feeds.js',
      html.includes('<script src="reading-feeds.js?v=122"></script>'));
