/**
 * test/test-stories-v103.js — Stories module (stories.js)
 * ============================================================
 * Run:  node test/test-stories-v103.js
 *
...
// ─── 6b. Prose rendering and density ────────────────────────

section('6b. Prose view and target-word density');
{
    const { S, DB } = loadModule();
    seedWords(DB, 20);
    S.createBatch({ count : 20, groupSize : 20 });
    S.applyResponse(JSON.stringify({ stories : [{ seq : 1, title : 'T', sentences : [
        { en : 'She pried the crate open.',  zh : '\u5979\u6253\u5f00\u4e86\u7bb1\u5b50\u3002' },
        { en : 'A w001 rolled out.',         zh : '\u4e00\u4e2a\u6ede\u4e86\u51fa\u6765\u3002' },
        { en : 'Later that week it rained.', zh : '\u90a3\u5468\u540e\u6765\u4e0b\u96e8\u4e86\u3002', br : true }
    ] }] }));

    const st = S.load()[0];
    eq(st.sents.length, 3, 'all three sentences stored');
    ok(st.sents[2].br === true, 'the paragraph-break flag is kept');
    ok(!('br' in st.sents[0]), 'and is absent where the model did not send it');

    // A reply with no br flags at all must still work: everything written
    // before v103 has none, and it renders as a single paragraph.
    const { S : S2, DB : DB2 } = loadModule();
    seedWords(DB2, 20);
    S2.createBatch({ count : 20, groupSize : 20 });
    S2.applyResponse('{"seq":1,"sentences":["One.","Two."]}');
    ok(!S2.load()[0].sents.some(x => x.br), 'no flags means no forced breaks');

    // Density: the suggested length has to scale with the words per piece, or
    // 20 targets land in 120 words and the model can only write a list.
    const prompt20 = S.buildPrompt(S.load(), { type : 'story', level : 'C1', length : 360 });
    ok(/about 360 words/.test(prompt20), 'the length reaches the prompt');
    ok(/18 words of prose per target word/.test(prompt20),
       'and the prompt states the density it implies');
    ok(/ONE continuous piece, not a set of example sentences/.test(prompt20),
       'coherence is stated as a requirement');
    ok(/Do NOT give each target word its own sentence/.test(prompt20),
       'and the failure mode is named explicitly');
    ok(/"br"/.test(prompt20), 'paragraph breaks are requested');
}
