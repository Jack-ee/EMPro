/**
 * test/test-stories-dom-v101.js — Stories UI flow (stories.js + index.html)
 * ============================================================
 * Run:  npm install jsdom  &&  node test/test-stories-dom-v101.js
 *
...
ok(/No material yet/.test($('sy-list').textContent), 'empty state invites the first run');
...
// 5. Paste back
$('sy-paste').dispatchEvent(new w.Event('click'));
...
ok($('sy-main').className.indexOf('sy-hidden') >= 0, 'list hidden while reading');
...
ok($('sy-main').className.indexOf('sy-hidden') < 0, 'back returns to the list');
